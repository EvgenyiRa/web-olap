...
$(document).ready(function() {
	...
	renderer.setSize(width, height);
        ...
        /*var dirLighth = new THREE.DirectionalLightHelper( dirLight, 5 );
        scene.add( dirLighth );*/
        
        /*var pointLight = new THREE.PointLight( 0xeef9ed, 0.5 );
        pointLight.position.set( 0, 0, 20 );
        scene.add( pointLight );
        
        var pointLightHelper = new THREE.PointLightHelper( pointLight, 50 ,0x000000);
        scene.add( pointLightHelper );*/
        
        /*var spotLight = new THREE.SpotLight( 0xffffff,1);
        spotLight.position.set( -220, 50, -150 );
        spotLight.castShadow = true;
        scene.add(spotLight );
        
        var spotLightHelper = new THREE.SpotLightHelper( spotLight,0x000000 );
	...
	scene.add(axes);*/
        ...
        var t_0_x=-70,
            ...
            //t_0_x_pok=t_0_x+(t_max_x-t_0_x)/2,
            t_0_x_pok=t_0_x-7,
            t_0_y_pok=98,
            ...
            //t_0_x_str=t_0_x+(t_max_x-t_0_x)/2,
            t_0_x_str=t_0_x-7,
            t_0_y_str=98,
            t_0_z_str=t_0_z-10,
            t_max_x_str=95,
        ...
        var MyIntervalID_X = setInterval(function(){
            ...
            if (!!font) {
                ...
                var MyInt_LoadData = setInterval(function(){
                    //console.log('задержка');
                    if (!!$(container).attr('id')) {
                        clearInterval (MyInt_LoadData);
                        var id_t=$(container).attr('id');
                        var table_par=$(window.opener.document).find(table_tag);
                        var rep_tab=$(table_par).find('td[olap_tab_id='+id_t+']');
                        var parend_td=$(table_par).find('div[olap_id='+id_t+']').closest('td');
                        var begin_tab_td=$(parend_td).closest('tr').next().find('td:eq('+$(parend_td).index()+')');
                        ...
                        var mass_max_pok=[];
                         ...
                         $(rep_tab).filter('[olap_td_class="td_val_name"]').each(function(i,elem) {
                            j=0;
                            //console.log(elem);
                            ...
                            j+=1;
                            $(rep_tab).closest('tr').filter('tr[olap_tr_class_'+id_t+'="tr_pok"]:not(tr[olap_tr_class_'+id_t+'="null"])').each(function(i2,elem2) {                                                                                                    
                                //console.log(elem2);
                            ...
                            $(rep_tab).closest('tr').filter('tr[olap_tr_class_'+id_t+'="tr_tab"]').each(function(i,elem2) {
                                ...
                                if ((!!!mass_max_zn_pok[$(elem).attr('olap_td_id')]) & (!isNaN(tek_elem))) {
                                    //console.log(tek_elem);
                                    mass_max_zn_pok[$(elem).attr('olap_td_id')]=tek_elem;
                                }
                                else if ((!!mass_max_zn_pok[$(elem).attr('olap_td_id')]) & (!isNaN(tek_elem)) & (tek_elem>mass_max_zn_pok[$(elem).attr('olap_td_id')])) {
                                    mass_max_zn_pok[$(elem).attr('olap_td_id')]=tek_elem;
                                }
                                if ((!!!mass_min_zn_pok[$(elem).attr('olap_td_id')]) & (!isNaN(tek_elem))) {
                                    //console.log(tek_elem);
                                    mass_min_zn_pok[$(elem).attr('olap_td_id')]=tek_elem;
                                }                                
                                else if ((!!mass_min_zn_pok[$(elem).attr('olap_td_id')]) & (!isNaN(tek_elem)) & (tek_elem<mass_min_zn_pok[$(elem).attr('olap_td_id')])) {
                                    mass_min_zn_pok[$(elem).attr('olap_td_id')]=tek_elem;
                                }  
                            ...
                            /*if (!!!mass_max_zn_pok[$(elem).attr('olap_td_id')]) {
                                mass_max_zn_pok[$(elem).attr('olap_td_id')]=mass_zn_tek.filter((x, i, a) => ((a.indexOf(x) == i) & (x!='')));
                            }*/
                        ...
                        /*mass_pok_vector.forEach(function(element,index) {                            
                            ...
                            //console.log(element);
                            xdel_group[index]=[];
                            xdel_mesh_group[index]=[];
                            for (var i = (count_pok_one-1); i >=1 ; i--) {
                                var text=String(element[i]);                                
                                if (text!==String(mass_pok_three_tek[i])) {
                                    //console.log(text);
                                    pr_uzel=true;                                    
                            ...
                            if (pr_uzel) {
                                ...
                                if (i!=(count_pok_one-1)) {
                                    for (var i2 = (count_pok_one-1); i2 >i; i2--) {
                                        t_k_group_z+=String(element[i2]).length*size_pok/1.1;
                                    }    
                                }
                                for (var i2 = i; i2 >=1; i2--) {
                                    var text='* '+String(element[i2]);
                                    //console.log(mass_pok_three_tek);
                                    xdel_group[index][i2] = new THREE.Group();                
                                    //xdel_group[index][i2].pok_index=index;
                                    if (text) {
                                        xdel_mesh_group[index][i2]=createText(text,size_pok);
                                        xdel_group[index][i2].add(xdel_mesh_group[index][i2]);                                    
                                    }   
                                    xdel_group[index][i2].rotation.y=-0.5*Math.PI;
                                    xdel_group[index][i2].position.x = t_0_x_pok;
                                    xdel_group[index][i2].position.y = t_k_group_y;
                                    xdel_group[index][i2].position.z = t_k_group_z+((text.length)*size_pok/2/1.1);
                                    
                                    xdel_group[index][i2].pok_index = index;
                                    xdel_group[index][i2].pok_i2 = i2;                                    
                                    scene.add(xdel_group[index][i2]);
                                    t_k_group_z+=String(element[i2]).length*size_pok/1.1;
                                    t_k_group_y-=sh_pok_group_y;
                                }    
                            }
                            else {
                                for (var i2 = (count_pok_one-1); i2 >=1 ; i2--) {
                                    var text=String(element[i2]); 
                                    t_k_group_z+=text.length*size_pok/1.1;
                                }
                            }                            
                            
                            i=0;                             
                            xdel_group[index][i] = new THREE.Group();                
                            xdel_group[index][i].pok_index=index;
                            var text=element[i];
                            if (text) {
                                xdel_mesh_group[index][i]=createText(text,size_pok);
                                xdel_group[index][i].add(xdel_mesh_group[index][i]);                                    
                            }   
                            xdel_group[index][i].rotation.y=-0.5*Math.PI;
                            xdel_group[index][i].position.x = t_0_x_pok;
                            xdel_group[index][i].position.y = t_k_group_y;
                            xdel_group[index][i].position.z = t_k_group_z+((text.length)*size_pok/2/1.1);
                            scene.add(xdel_group[index][i]);        
            ...
            if (!!font) {
                ...
                var MyInt_LoadData = setInterval(function(){
                    ...
                    if (!!$(container).attr('id')) {
                        ...
                        var mass_min_str=[];
                        var mass_max_str=[];
                        $(rep_tab).closest('tr').filter('tr[olap_tr_class_'+id_t+'="tr_tab"]').each(function(i,elem) {
                            //console.log(elem);
                            mass_str[i]='';
                            mass_str_vector[i]=[];
                            $(elem).find('td[olap_td_class="td_str_val"]').each(function(i2,elem2) {
                            ...
                            $(elem).find('td[olap_td_class="td_val_val"]').each(function(i2,elem2) {
                                ...
                                if (!!mass_max_zn_pok[$(elem2).attr('olap_td_id')]) {   
                                    if (Math.abs(mass_max_zn_pok[$(elem2).attr('olap_td_id')])>Math.abs(mass_min_zn_pok[$(elem2).attr('olap_td_id')])) {
                                        height_max=mass_max_zn_pok[$(elem2).attr('olap_td_id')];
                                    }
                                    else {
                                        height_max=mass_min_zn_pok[$(elem2).attr('olap_td_id')];
                                    }
                                    if (height_max===0) {
                                        height_max=1;
                                    } 
