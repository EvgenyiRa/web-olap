...
var methods = {
    ...
    createTable : function() {
        ...
        // Create headers
        tr = '<td width="40" class="jexcel_label"></td>';
        cg = '<col width="40">';
        ...
        if (! $('.jexcel_corner').length) {
                ...
                if ($.fn.jexcel.current) {
                    ...
                    if ($(table).is('.jexcel')) {
                        ...
                        if ($(e.target).parent().parent().is('thead')) {
                            ...
                            var o_td_column=o.split('-')[1];
                        } else {
                            var o = $(e.target).parent().prop('id');
                            var o_td_column=$(e.target).prop('id').split('-')[0];
                        }                        
                            ...
                            if (typeof($.fn.jexcel.defaults[$.fn.jexcel.current].contextMenu) == 'function') {
                                ...
                                if ($(e.target).parent().parent().is('thead')) {
                                    ...
                                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowInsertColumn == true) {
                                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('insertColumn', 1, null, " + o_td_column + ")\">Insert a new column<span></span></a>";
                                    }
            ...
            $.fn.jexcel.mouseDownControls = function (e) {
                ...
                //если не работа с панелью
                if (($(e.target).closest('div.div_action_one').length==0) & ($(e.target).closest('div.modal_content').length==0)) {
                    ...
                    if (e.target.id == 'jexcel_corner') {
                        ...
                        if ($(table).is('.jexcel')) {
                            ...
                            if ($(e.target).parent().parent().is('thead')) {
                                ...
                                if (o) {
                                    ...
                                    if ($(e.target).outerWidth() - e.offsetX < 8) {
                                        ...
                                        var o1_num_true=1;
                                        while ($(o1).length==0) {
                                            o1 = $('#' + $.fn.jexcel.current).find('#' + o[1] + '-'+o1_num_true);
                                            o1_num_true+=1;
                                        }
                                        var o2_num_true=parseInt($.fn.jexcel.defaults[$.fn.jexcel.current].data.length - 1);
                                        var o2 = $('#' + $.fn.jexcel.current).find('#' + d[1] + '-' + o2_num_true);
                                        o2_num_true-=1;
                                        while ($(o2).length==0) {
                                            o2 = $('#' + $.fn.jexcel.current).find('#' + o[1] + '-'+o1_num_true);
                                            o2_num_true-=1;
                                        }
                            ...
                            else if (($(e.target).outerHeight() - e.offsetY < 8) & ($(e.target).index()==0) ) {
                                if ($.fn.jexcel.defaults[$.fn.jexcel.current].columnResize == true) {
                                        // Resize helper
                                        $.fn.jexcel.resizeColumn = {
                                            mousePosition: e.pageX,
            ...
            $.fn.jexcel.mouseUpControls = function (e) {
                ...
                if ($(e.target).closest('div.modal_content').length>0) {
                    return;
                }
                    ...
                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].columnResize == true) {
                        ...
                        if ($.fn.jexcel.resizeColumn) {
                           ...
                           if ($.fn.jexcel.resizeColumn.width + width > 0) {
                               ...
                               $('.slide_panel #td_width').val($.fn.jexcel.resizeColumn.width + width);
                           }
                                                      
                           var height=e.pageY - $.fn.jexcel.resizeColumn.mousePositionY+$.fn.jexcel.resizeColumn.height+8;
                           if (height > 27) {
                               $('#' + $.fn.jexcel.current+' tbody tr:eq('+$.fn.jexcel.resizeColumn.rowIndex+')').attr('height',height);
                               $('.slide_panel #td_height').val(height);
                           }
                        } else {
                            // Header found
                            if ($(e.target).parent().parent().is('thead')) {
                            ...
                            else if (($(e.target).index()==0) & ($(e.target).is('td'))) {
                                if ($(e.target).outerHeight() - e.offsetY < 8) {
            ...
            $.fn.jexcel.pasteControls = function (e) {
                if (($(e.target).closest('div.container').length>0) 
                    || ($(e.target).closest('div.slide_panel').length>0)
            ...
            $.fn.jexcel.keyDownControls = function(e) { 
                //console.log(e.target);
                //зарезервированные конпки системы
                if (([121,120,119,118,115].indexOf(e.which)!=-1) /*|| (tab_obj.pr_view)*/) {        
                    e.preventDefault();
                }
                else if ((!tab_obj.pr_view) & (!$(modal_form).is(':visible'))) {                                      
                    if (($(e.target).closest('div').attr('class')!='div_action_one') & (!$(e.target).is('.in_param_val'))
                         & ($(e.target).closest('div.container').length==0) & (!$(e.target).is('.input_add'))  
                         & (!$(e.target).is('.tab_input')) & (!$(e.target).is('input.form-control.multiselect-search'))) {
                        if ($.fn.jexcel.current) {
                            // Support variables
                            var cell = null;
                            // Get current cell
     ...
     * @return void
     */
    updateData : function(data,colHeaders) {
        // Id
        var id = $(this).prop('id');
        ...
        if (colHeaders) {
            $.fn.jexcel.defaults[id].colHeaders = colHeaders;
        }
    },
    ...
    openEditor : function(cell, empty) {
        // Id
        if (($(cell).find('#group_tab').length>0) || ($(cell).find('.settings_group_panel').length>0) || pr_td_no_edite) {
            return;
        }
        var id = $(this).prop('id');
    ...
    updateSelection : function(o, d, origin) {
        ...
        if (options.active_cells[1]) {            
            options.active_cells.forEach(function(element) {
                $(element).removeClass('highlight');
                if (!tab_obj.pr_view) {
                    $(element).css({'-webkit-filter':'','filter':''});  
                }    
            });
            options.active_cells.length = 0;
        }
        ...
        if (($(o).length==1) & ($(d).length==1)) {
            ...
            for (var i = px; i <= ux; i++) {
                for (var j = py; j <= uy; j++) {
                    // Select markers
                    var tek_td=$(this).find('#' + i + '-' + j);  
                    count_ac+=1;
                    options.active_cells[count_ac]=tek_td;
                    if (!tab_obj.pr_view) {
                        var tek_td_params_group=$(tek_td).find('.params_group'),
                            select_add_length=$(tek_td).find('.select_add,.img_add,.input_add,.in_modal_add_val,.panel_add,div[olap_id]').length;
                        if (($(tek_td_params_group).length===0) & (select_add_length===0)) {
                            $(tek_td).addClass('highlight').css({'-webkit-filter':'invert(0.05)','filter':'invert(0.05)'});
                        }
                        else if ((select_add_length>0) || ($(tek_td_params_group).length>0)) {
                            $(tek_td).addClass('highlight');
                        }
                    }
                    else {
                        $(tek_td).addClass('highlight');
    ...
    getSelectedCells : function() {
        // Get selected cells
        if ($.fn.jexcel.current) {
            //return $('#' + $.fn.jexcel.current).find('tbody td[style*="filter: invert(0."]');
            return $('#' + $.fn.jexcel.current).find('.highlight');            
        }
    },
    ...
    paste : function(cell, data) {
        if ($(cell).find('#group_tab').length>0) {
            return;
        }
        // Id
        var id = $(this).prop('id');
    ...
    insertColumn : function (mixed, properties, columnNumber) {
        ...
        var options = $.fn.jexcel.defaults[id];
        if (!!!properties) {
            properties={updSelection:true,in_end:false,not_log:false};
        }
        if (isNaN(properties.in_end)) {
            properties.in_end=false;
        }
        if (isNaN(properties.updSelection)) {
            properties.updSelection=true;
        }
        if (isNaN(properties.not_log)) {
            properties.not_log=false;
        }
        ...
        if (options.allowInsertColumn == true) {
            ...
            if (direction == 1) {
                ...
                for (var row = 0; row < options.data.length; row++) {
                    ...
                    // Reference
                    var referenceColumn
                    if (properties.in_end) {
                        referenceColumn = $(this).find('tbody').find('#row-' + row+' td:last');
                    }
                    else {
                        referenceColumn = $(this).find('tbody').find('#row-' + row).find('.c' + (columnNumber - 1));
                ...
                if (!properties.in_end) {
                    ...
                    for (var col = (numOfColumns + columnNumber-1); col >= columnNumber; col--) {
                        var title = options.colHeaders[col+1];
                        var header = title ? title : $.fn.jexcel('getColumnName', (col+1));
                        var display = (options.columns[col+1].type == 'hidden') ? ' style="display:none"' : '';
                        td = '<td id="col-'+(col+1)+'" width="' + options.colWidths[col] + '" align="' + options.colAlignments[col] + '" title="' + title + '" class="' + options.colHeaderClasses[col+1] + '"' + display + '>' + header + '</td>';
                        $(referenceHeader).after(td);
                    }
                }
                ...
                for (var row = 0; row < options.data.length; row++) {
                    ...
                    if (properties.in_end) {
                        ...
                        referenceColumn = $(this).find('tbody').find('#row-' + row).find('.c' + (columnNumber));
                        if ($(referenceColumn).length==0) {
                            referenceColumn = $(this).find('tbody').find('#row-' + row+' td:last');
                        }
                    }
                    ...
                    if (!properties.in_end) {
                        ...
                        for (var col = (numOfColumns + columnNumber-1); col >= columnNumber; col--) {
                            td = $(this).jexcel('createCell', (col+1), row);
                            $(referenceColumn).after(td);
                            // New value
                            $.fn.jexcel.defaults[id].data[row][col+1] = data[row] ? data[row] : '';
                            // Cell data
                            records.push({
                                col: (col+1),
                                row: row,
                                cell: $(td),
                                newValue: $.fn.jexcel.defaults[id].data[row][col+1],
                                oldValue: '',
                            });
                        }                        
            ...
            // Events
            if (($.fn.jexcel.ignoreEvents != true) & (!properties.not_log)) {
     ...
     * @return void
     */
    insertRow : function(mixed, rowNumber, properties) {
        // Main
        var main = $(this);
        ...
        if (options.allowInsertRow == true) {
            ...
            // Creat row closure
            var createRow = function(count_col) {
                // New line of data to be append in the table
            ...
            if (direction == 1) {
                for (var row = (numOfRows + rowNumber - 1); row >= rowNumber; row--) {
                    $(referenceRow).after(createRow($.fn.jexcel.defaults[id].colHeaders.length));
                }
            } else { 
                var col_count=$.fn.jexcel.defaults[id].colHeaders.length;
                if (!properties.not_log) {
                    //col_count=$(referenceRow).find('td').last().index()+1;                         
                    //var referenceRow_td=$(referenceRow).find('td:gt(0)');
                    //случаи когда rowspan находится выше текущей строки, а ячейки ниже все равно добавить надо, т.к. все сдвинется на одну строку
                    var tek_colspan_add=0;
                    var tek_colspan_add_index=0;
                    var tek_colspan_add_prev=0;
                    var tek_colspan_add_index_prev=0;
                    var count_rowspan=0,count_rowspan_last=0,pr_up=true,referenceRow_calc=referenceRow;
                    while (pr_up) {
                        //проверяем кол-во rowspan поступивших от строк выше, идем на самый верх, м.б. цепочка, запоминаем 
                        for (var i2 = 0; i2 < ($.fn.jexcel.defaults[id].colHeaders.length); i2++) {
                            if (tek_colspan_add>0) {
                                //пропускаем colspan, не интересны для данного случая
                                if (i2>(tek_colspan_add+tek_colspan_add_index)) {
                                    tek_colspan_add=0;
                                    tek_colspan_add_index=0;
                                }
                                else {
                                    continue;
                                }
                            }  
                            if (tek_colspan_add_prev==0) {
                                var tek_td=$(referenceRow_calc).find('td[id^="'+i2+'-"]');
                                if ($(tek_td).length>0) {
                                    var tek_colspan=$(tek_td).attr('colspan')
                                    if (tek_colspan>1) {
                                        tek_colspan_add=tek_colspan;
                                        tek_colspan_add_index=i2;
                                    }
                                }
                                else {
                                    count_rowspan+=1;
                                    var tr_rowspan=$(referenceRow_calc).prev();
                                    var tr_rowspan_td=$(tr_rowspan).find('td[id^="'+i2+'-"]');
                                    while ($(tr_rowspan_td).length==0) {
                                        tr_rowspan=$(tr_rowspan).prev();
                                        tr_rowspan_td=$(tr_rowspan).find('td[id^="'+i2+'-"]');
                                    }
                                    var td_colspan_val=Number($(tr_rowspan_td).attr('colspan'));
                                    if (td_colspan_val>1) {
                                        tek_colspan_add_prev=td_colspan_val;
                                        tek_colspan_add_index_prev=i2;
                                    }
                                    if (Number($(tr_rowspan).attr('id').split('-')[1])<Number($(referenceRow_calc).attr('id').split('-')[1])) {
                                        referenceRow_calc=tr_rowspan;
                                    }
                                }
                            }
                            else {
                                if (i2>(tek_colspan_add_prev+tek_colspan_add_index_prev)) {
                                    tek_colspan_add_prev=0;
                                    tek_colspan_add_index_prev=0;
                                }
                                else {
                                    continue;
                                }
                        ...
                        if (count_rowspan==0) {
                            ...
                            count_rowspan_last=count_rowspan;
                            count_rowspan=0;
                            tek_colspan_add=0;
                            tek_colspan_add_index=0;
                            tek_colspan_add_prev=0;
                    ...
                    if (count_rowspan_last>0) {
                        referenceRow=referenceRow_calc;
                        //col_count=$.fn.jexcel.defaults[id].colHeaders.length;
                    }                    
                } 
                for (var row = rowNumber; row < (numOfRows + rowNumber); row++) {
                    var tr_insert=createRow(col_count);                    
                    $(referenceRow).before(tr_insert);
                    var prev_v=$(tr_insert).prev(),
                        tr_index;
                    if ($(prev_v).length>0) {
                        tr_index=Number($(prev_v).attr('id').split('-')[1])+1;
                    }
                    else {
                        tr_index=0;
                    }
                    if (!tab_obj.pr_view) {
                        $(tr_insert).find('td').first().html((Number(tr_index)+1));
                    }
                    else {
                        $(tr_insert).find('td').first().css('empty-cells','hide');
                    }
                    $(tr_insert).attr('id','row-'+tr_index);
                }
            }
            ...
            if (!properties.not_log) {
                // Remove table references
                var num_row=Number($(referenceRow).attr('id').split('-')[1])-1;
                if (num_row<0) {
                    num_row=0;
                }
                $(this).jexcel('updateTableReferences',0,num_row);
    ...
    deleteColumn : function(columnNumber, numOfColumns) {
        ...
        if (options.allowDeleteColumn == true) {
            ...
            var pr_calc_colspan=false;
            $(tbody_tr).first().find('td:gt(0)').each(function(i,elem) {
                if ($(elem).attr('colspan')>1) {
                    pr_calc_colspan=true;
                    return false;
                }
            });
            if ((options.data[0].length > 1) || (pr_calc_colspan)) {
                if (parseInt(columnNumber) > -1) {
                    ...
                    for (var col = columnNumber; col < columnNumber + numOfColumns; col++) {
                        ...
                        // Remove column
                        //$(this).find('tbody .c' + col).remove(); будем удалять построчно, т.к. могут попадаться объединенные ячейки
                        var tek_rowspan_max=0;
                        $(tbody_tr).each(function(i,elem) {
                            var tek_td=$(elem).find('.c' + col);
                            if ($(tek_td).length>0) {
                                $(tek_td).remove();
                            }
                            else {
                                //ищем ближайший с colspan>1 
                                if (tek_rowspan_max==0) {
                                    for (var i = (col-1); i >= 0; i--) {
                                        var tek_td_for=$(elem).find('.c' + i);
                                        if ($(tek_td_for).length>0) {
                                            var tek_colspan=Number($(tek_td_for).attr('colspan'));
                                            $(tek_td_for).attr('colspan',(tek_colspan-1));
                                            var tek_rowspan=Number($(tek_td_for).attr('rowspan'));
                                            if (tek_rowspan>1) {
                                                tek_rowspan_max=Number($(elem).attr('id').split('-')[1])+tek_rowspan-1;
                                            }
                                            break;
                                        } 
                                    }
                                    ...
                                    var tek_index=Number($(elem).attr('id').split('-')[1]);
                                    if (tek_rowspan_max>tek_index) {
                                        tek_rowspan_max=0;
                                    }
                                }
                            }
                ...
                //для корректной отрисовки olap-кубов
                tab_obj.html=$(this).html();
            } else {
     ...
     * @return void
     */
    deleteRow : function(rowNumber, numOfRows, option) {
        // Id
        var id = $(this).prop('id');
        if (!!numOfRows) {
        ...
        if (options.allowDeleteRow == true) {
            if (!!option) {
                if (isNaN(option['not_log'])) {
                    option['not_log']=false;
                }    
            }
            else {
                option={};
                ...
                if (parseInt(rowNumber) > -1) {
                    ...
                    if (!option['not_log']) {
                        for (var row = rowNumber; row < (rowNumber + numOfRows); row++) {
                            ...
                            var td_all=$(line).find('td:gt(0)');                            
                            var max_rowspan=1,max_rowspan_col=0;
                            var line_dop;
                            
                            //признак расчета из-под кубов
                            if (!option['not_log']) { 
                                for (var i = 0; i <= ($.fn.jexcel.defaults[id].columns.length-1); i++) {
                                    var tek_td=$(line).find('td[id^="'+i+'-"]');
                                        if ($(tek_td).length==0) {
                                            alert('Запрещено удаление строк на объединенных ячейках (станет возможно после объединения)');
                                            return true;
                                        }        
                                }
                            }    
                            $(td_all).each(function(i,elem) {
                                var tek_rowspan=Number($(elem).attr('rowspan'));
                                if (tek_rowspan>max_rowspan) {
                                    line_dop=line;
                                    var tek_colspan=Number($(elem).attr('colspan'));
                                    if (isNaN(tek_colspan)) {
                                        tek_colspan=1;
                                    }
                                    var coord=$(elem).attr('id').split('-');
                                    max_rowspan_col=Number(coord[0])+tek_colspan;
                                    for (var i = 1; i <= (tek_rowspan-1); i++) {
                                        line_dop=$(line_dop).next();
                                        var td_before=$(line_dop).find('.c'+max_rowspan_col);
                                        var pr_before=true;
                                        if ($(td_before).length==0) {
                                            td_before=$(line_dop).find('td').last();
                                            pr_before=false;
                                        }
                                        var td_new='';
                                        var line_dop_coord=$(line_dop).attr('id').split('-');
                                        for (var j = 1; j <= (tek_colspan); j++) {
                                            var td_ind=coord[0]+j-1;                    
                                            td_new+='<td id="'+td_ind+'-'+line_dop_coord[1]+'" class="c'+td_ind+' r'+line_dop_coord[1]+'"></td>';
                                        } 
                                        if (pr_before) {
                                            $(td_before).before(td_new);
                                        }
                                        else {
                                            $(td_before).after(td_new);
                                        }
                                    }
                                }                                                                                
                            });
                        ...
                        var line = $(this).find('#row-' + rowNumber);
                        for (var row = 1; row <= numOfRows; row++) {
                            var line_next=$(line).next();
                            $(line).remove();
                            line=line_next;
                        }    
                    }
                    ...
                    // Remove table references
                    $(this).jexcel('updateTableReferences', 0, rowNumber,option);
     ...
     * @return void
     */
    updateTableReferences : function(referenceCol, referenceRow, option) {
        // Main
        var main = $(this);
        
        var mass_index_cols=[],mass_left_cols=[];
        $(main).find('thead tr:last td:gt(0)').each(function(i,elem) {
        ...
        var rows;
        if (referenceRow>0) {
            rows = $(this).find('tbody > tr:gt('+(parseInt(referenceRow)-1)+')');
        }
        else {
        ...
        $.each(rows, function(k, v) {
            //if (k >= referenceRow) {
                ...
                var k_true=k+referenceRow;
                $(v).prop('id', 'row-' + k_true);
                ...
                $.each(columns, function(k1, v1) {
                    ...
                    if (k1 == 0) {
                        ...
                        if (k1 >= referenceCol) {
                            ...
                            //$(v1).prop('id', (k1 - 1) + '-' + k);
                            var k1_true=mass_left_cols[$(v1).offset().left];
                            $(v1).prop('id', (k1_true - 1) + '-' + k_true);                            

                            // Update row
                            $(v1).removeClass('r' + coord[1]);
                            $(v1).addClass('r' + k_true);

                            // Update col
                            $(v1).removeClass('c' + coord[0]);
                            $(v1).addClass('c' + (k1_true - 1));

                            // Formula?
                            var val = '' + $.fn.jexcel.defaults[id].data[k_true][k1_true - 1];

                            if (val.substr(0, 1) == '=') {
                                // Update formula references
                                if (coord[0] != k1_true - 1) {
                                    // Update formulas
                                    val = $.fn.jexcel('shiftFormulaByColumn', val, k1_true - 1 - coord[0]);

                                    // Update value on the cell
                                    $.fn.jexcel.defaults[id].data[k_true][k1_true - 1] = val;
                                    $(v1).find('input').val(val);
                                }

                                if (coord[1] != k_true) {
                                    // Update formulas
                                    val = $.fn.jexcel('shiftFormulaByRow', val, k_true - coord[1]);
                            ...
                            if (!option['not_log']) {
                                var letter = $.fn.jexcel('getColumnNameFromId', [k1_true - 1, k_true]);
